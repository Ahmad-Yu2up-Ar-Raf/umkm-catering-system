...
Behavior contract: exports of ANY module with `rows <= 2000` build inline in `store()` (same 202 shape, first poll already `ready`/`failed`, no worker needed); only larger datasets dispatch to the queue. A dispatch failure degrades to inline (small) or terminal `failed` (large) — never a poisoned `pending`. `start.sh` runs `migrate --force` on boot so the worker always has its `jobs` table. Image cells are **always HYPERLINK text** (thumbnail embedding disabled via `IMAGE_EMBED_MAX_ROWS = 0`: the PhpSpreadsheet drawing path held up to 1200 Drawing/GD resources in RAM until `save()` and SIGKILLED the 512M worker, freezing polls at `processing`). All exports stream O(1) memory with zero Cloudinary fetches, chunked at 100 rows with a 400M graceful memory guard (clean `failed` status instead of a silent kill). Job budget: `$timeout=900`, `$tries=1`; worker: `queue:work database --timeout=1000 --memory=512`; `retry_after` defaults to 1100 (must exceed worker timeout; also set `DB_QUEUE_RETRY_AFTER=1100` in prod env). Octane HTTP cap 30s is why exports never build on the HTTP worker. Frontend polls with backoff 1→10s: pending budget 30s text / 120s image, stall budget 45s text / 120s image (fresh `heartbeat_at` < 60s proves liveness), absolute deadline 6min text / 12min image.
