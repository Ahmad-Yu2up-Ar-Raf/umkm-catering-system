...
> Level-100 quick-start for AI agents working inside `Back-End/`. Read the modular docs (below) for deep context. This file is the fast-loading overview.
...
7. **Zero-Hallucination pipeline** with every feature (see WORKFLOW.md): code → Pest GREEN → Bruno → Scramble.
...
| File | Purpose |
|------|---------|
| `WORKFLOW.md` | Zero-Hallucination pipeline: Code → Pest → Bruno → Postman → Scramble + pre-flight checklist |
| `ARCHITECTURE.md` | Folder structure, flat controllers (LOCKED), Enums/Requests/Resources/Services layers, routes, response envelope, code conventions |
| `DATABASE.md` | Full DBML schema, JSON array rules, critical business rules (pricing, struk, Tumpeng Mini) |
| `BOOST-GUIDELINES.md` | Laravel Boost MCP tooling guidelines (auto-generated block, relocated here) |

## Cross-Reference (repo root)

- `ARCHITECTURE.md` — sitemap & monorepo-wide ERD
- `DESIGN.md` — design tokens (Front-End)
- `GIT-WORKFLOW.md` — branch/PR/commit conventions
