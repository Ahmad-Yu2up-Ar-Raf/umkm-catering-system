...
import {
  usePaketViewStore,
  type PaketAdminViewMode,
} from "@/store/paket-admin-view-store"

/** Luxe tween — shared easing family with the catalog toggle (2px magnet feel). */
const LUXE_EASE = [0.16, 1, 0.3, 1] as const

const MODES: { mode: PaketAdminViewMode; icon: typeof Menu01Icon; label: string }[] = [
  { mode: "table", icon: Menu01Icon, label: "Tabel" },
  { mode: "grid", icon: GridViewIcon, label: "Grid" },
]

/** Admin paket view switcher — reads/writes the persisted `paket-admin-view-store`. */
export function PaketViewToggle() {
  const viewMode = usePaketViewStore((s) => s.viewMode)
  const setViewMode = usePaketViewStore((s) => s.setViewMode)

  return (
    <div
      role="group"
      aria-label="Tampilan daftar paket"
      ...
      {MODES.map(({ mode, icon: Icon, label }) => {
        ...
        return (
          ...
          >
            {active && (
              <motion.span
                layoutId="paket-admin-view-mode"
