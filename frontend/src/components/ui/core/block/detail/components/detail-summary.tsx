...
export function DetailSummary({ vm }: { vm: DetailViewModel }) {
  ...
  return (
    <>
      <div className="flex w-full flex-col gap-6 px-6 md:gap-6 md:p-0 lg:py-3">
        ...
        {vm.metaRows.length > 0 && (
          <>
            ...
            <motion.div
              {...reveal()}
              className="flex flex-col gap-3.5 md:gap-6"
            >
              {vm.metaRows.map((row) => (
      ...
      {isMobile && (
        <Dialog open={orderOpen} onOpenChange={setOrderOpen}>
          <DialogTrigger className="bg- sticky bottom-0 bg-background px-6 pt-2 pb-2">
            <OriginButton
              intensity={0.8}
              range={120}
              className="group w-full border border-primary/40 bg-primary text-xs tracking-widest text-primary-foreground uppercase sm:border-2 sm:border-primary md:bg-transparent md:text-primary"
            >
              <HugeiconsIcon icon={ShoppingCart} className="size-5" />
              Pesan Sekarang
            </OriginButton>
